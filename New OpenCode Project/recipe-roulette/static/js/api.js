const API = {
    ...
    discoverRecipes(count = 12, search) {
        var params = 'count=' + count;
        if (search) params += '&search=' + encodeURIComponent(search);
        return this.request('/api/recipes/discover?' + params);
    },

    getRandomRecipes(count = 12) {
        return this.request(`/api/recipes/random?count=${count}`);
    },
